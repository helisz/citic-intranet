<?php
function abcfsl_mbox_mfilter_cat( $tplateID, $filterOptns, $filterNo ) {

    echo abcfl_input_sec_title_hlp( ABCFSL_ICONS_URL, abcfsl_txta(240), abcfsl_aurl(36));

    abcfsl_mbox_mfilter_cat_all( $filterOptns, $filterNo );
    abcfsl_mbox_mfilter_cat_slugs_hdr();

    $slugs = get_post_meta($tplateID, '_catSlugs' . $filterNo, true);
    abcfsl_mbox_mfilter_cat_slugs( $slugs, $filterNo );
...
function abcfsl_mbox_mfilter_cat_all( $filterOptns, $filterNo ) {
...
function abcfsl_mbox_mfilter_cat_slugs_hdr() {
...
function abcfsl_mbox_mfilter_cat_slugs( $slugs, $filterNo ) {
    ...
    if ( $slugs ) {
        foreach ( $slugs as $field ) {
            ...
            echo abcfsl_mbox_mfilter_cat_slug_row( $fieldValue, $filterNo );
        }
    }
	else {
            // show a blank one
            echo abcfsl_mbox_mfilter_cat_slug_row( '', $filterNo );
    }
    // empty hidden one for jQuery
    echo abcfsl_mbox_mfilter_cat_slug_row( '', $filterNo, 'screen-reader-text slsTrEmptyRowSlug' . $filterNo );
...
function abcfsl_mbox_mfilter_cat_slug_row( $fieldValue, $filterNo, $cls='' ){
