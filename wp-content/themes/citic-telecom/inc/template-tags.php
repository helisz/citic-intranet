...
function web2feel_posted_on() {
	...
	if ( qtrans_getLanguage() == 'en' ) {
      	$date_by_lang = get_the_date( 'l F j, Y' );
    }
    else{
      	$date_by_lang = get_the_date( 'Y年Fj日, l' );
    }
	...
	$time_string = sprintf( $time_string,
		...
		esc_html( $date_by_lang ),
		esc_attr( get_the_modified_date( 'c' ) ),
		esc_html( get_the_modified_date() )
	);

	printf( __( '<span class="posted-on">%1$s</span><span class="byline">&nbsp;&nbsp;%2$s</span>', 'web2feel' ),		
